...
这是一个基于 **STM32F4** 标准库开发的按键驱动模块。它采用了**有限状态机 (FSM)** 架构与**数据驱动 (Data-Driven)** 的编程思想，实现了对物理按键的深度解耦与高效管理。
...
1. **KEY_RELEASE**: 初始松开状态，监听电平变化。
2. **KEY_CONFIRM**: 物理消抖阶段，确认按下有效性。
3. **KEY_SHORTPRESS**: 识别窗口期，区分“即将长按”还是“准备松手”。
4. **KEY_WAIT_DOUBLE**: 关键“观察期”，等待 250ms 内是否有第二次按下（判定双击）。
5. **KEY_LONGPRESS**: 长按锁定状态，防止重复触发。
